...
import com.mikeleith.mockery.Mockery;
...
public final class MockeryInject {
    ...
    private static Constructor<?> weigh(final Constructor<?>[] constructors, final List<Field> fields) {
        ...
        for (final Constructor<?> constructor: constructors) {
            ...
            for (final Parameter param : params) {
                ...
                weights += 1;
                while (i > 0 && fields.get(i - 1).getName().equals(param.getName())) { i--; }
    ...
    private static Object[] buildParams(final Object o,
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            int j = Collections.binarySearch(fields, clazz.getName(), MockeryInject::compareFieldToString);
            boolean match = false;
            final String name = params[i].getName();
            while (j > 0 && fields.get(j - 1).getName().equals(name)) { j--; }
            for (int k = j; k < fields.size() && clazz.equals(fields.get(k).getType()); k++) {
                if (fields.get(k).getName().equals(name)) {
